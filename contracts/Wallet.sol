...
contract Wallet {
    ...
    constructor(string memory _name, uint _numConfirmationsRequired, address[] memory _owners, string[] memory _ownerNames) payable {
        ...
        for(uint i = 0; i < _owners.length; i++) {
            ...
            isOwner[owner] = true;
            owners.push(owner);
            ownerNames[owner] = _ownerNames[i];
        }
    ...
    function addOwner(address _owner, string memory _ownerName) public onlyOwner {
        require(!isOwner[_owner], "Owner not unique");
        owners.push(_owner);
        ownerNames[_owner] = _ownerName;
    ...
    function getOwner(address _owner) external view returns (address owner, string memory ownerName) {
        return (
        _owner,
        ownerNames[_owner]
        );
